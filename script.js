...
console.log('[测测主队] script.js v5 加载中...');
...
async function startNewQuiz() {
  console.log('[测测主队] startNewQuiz 被调用');
  const saved = loadProgress();
  if (saved && saved.currentIndex > 0 && saved.currentIndex < QUESTIONS.length) {
...
// ==================== 初始化 ====================
function init() {
  console.log('[测测主队] init() 开始, QUESTIONS=' + QUESTIONS.length + ', CLUBS=' + CLUBS.length);
  STATE.traits = [0,0,0,0,0,0];
  STATE.hometown = '';
  totalNumEl.textContent = QUESTIONS.length;
  initStartPage();
  showPage(pageStart);
  console.log('[测测主队] init() 完成, 按钮=' + (btnStart ? 'OK' : 'NULL'));
}
init();
